<div class="border-end bg-white" id="sidebar-wrapper">
    ...
    <div class="list-group list-group-flush">
        @if (Auth::check())
            ...
            @endif
            <x-navlink-component link="{{ route('login') }}" icon='mdi mdi-store '>
                Materiels
            </x-navlink-component>
            <x-navlink-component link="{{ route('login') }}" icon='mdi mdi-account-multiple '>
                Personnels
            </x-navlink-component>
            <x-navlink-component class="border mt-1" link="{{ route('dashboard') }}" icon="mdi-dollar-usd">
                Financement
            </x-navlink-component>
            {{-- je met le button de deconnexion --}}
            <form action="{{ route('logout') }}" method="POST" onclick="submit()">
                @csrf
                <x-navlink-component class="border bg-outlined-danger text-white mt-2"  link="#!" icon="mdi-logout">
                    Se Deconnecter
                </x-navlink-component>
            </form>
            @else
